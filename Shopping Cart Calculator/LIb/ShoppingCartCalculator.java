package LIb;

import java.util.ArrayList;

public class ShoppingCartCalculator {
     /**
     * เขียน Javadoc ที่นี่เพื่ออธิบายกฎการทำงานและกรณีพิเศษ:
     * - จะทำอย่างไรถ้า items เป็น null หรือ empty?
     * - จะทำอย่างไรถ้า CartItem มี price หรือ quantity ติดลบ?
     * - กฎส่วนลด BOGO (ซื้อ 1 แถม 1)
     * - กฎส่วนลด BULK (ซื้อ >= 6 ชิ้น ลด 10%)
     */
    public static double calculateTotalPrice(ArrayList<CartItem> items) {
        double stok = 0 ;
        // TODO: เขียนโค้ด Implementation ที่นี่
        if (items == null || items.isEmpty()) {
            return 0.0;
        }
        for( CartItem CartItem : items){
            if (CartItem.sku() == "Boko" && CartItem.quantity() >= 2) {
                stok = CartItem.price() + CartItem.quantity() /2.0;
            }
            else if (CartItem.sku() == "Bulk" && CartItem.quantity() >= 3 ) {
                stok = CartItem.price() + CartItem.quantity()*(100/20);
            }
            else{
                stok = CartItem.price() + CartItem.quantity() ;
            }
        }
        return stok;
    }
}

     
